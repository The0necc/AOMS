...
function Save(){
    ...
    $.ajax({
        url:'/configration/template_modify/',
        type:'POST',
        traditional:true,
...
function Initialize(tBody,page){
    ...
    $.Show('#shade,#loading');
    $.ajax({
        url:'/configration/template_list/',
        type:'POST',
        traditional:true,
...
function EmbedIntoTable(response,startNum,body){
    if(!response.status){
        ...
        $.each(response.data,function(key,value){
            ...
            var d = $.CreateA({'href':'/configration/template_detail/'+value.id +'/','target':'_blank' },{},'详细');
