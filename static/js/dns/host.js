 ...
 */
$(function () {
    $('#left_menu_host').addClass('active');
...
function InitGlobalDict(callback){
    window.window_host_type = callback.type_choice.data;
}
...
function EmbedIntoTable(response,startNum,body){
    if(!response.status){
        ...
        $.each(response.data,function(key,value){
            ...
            tds.push($.CreateTd({'edit-enable':'false','edit-type':'input','name':'hostname','origin':value.hostname},{}, value.hostname));
            var type_text = TypeToText(value.host_type);
            tds.push($.CreateTd({'edit-enable':'false','edit-type':'select','value_key':'0','text_key':1,'name':'host_type','origin':value.host_type,'edit-option':'host_type','options':'window_host_type'},{}, type_text));

            var status_icon = $.CreateA({'href':'javascript:void(0);','target':'_blank', 'onclick':'FetchHostStatus(this,"'+ value.hostname+'")'},{},'<i class="loading-16"></>');
            tds.push($.CreateTds({},{},[status_icon],''));

            FetchHostStatus(status_icon, value.hostname);

            var d = $.CreateA({'href':'/dns/host_detail/'+value.id +'/','target':'_blank' },{},'详细');
...
function FetchHostStatus(loading, hostname){
    $(loading).html('<i class="loading-16"></>');
    $.ajax({
        url:'/dns/fetch_host_status/',
        async: true,
        type:'POST',
        ...
        success:function(callback){
            callback = $.parseJSON(callback);
            if(callback.status){
                $(loading).html('<i class="glyphicon glyphicon-ok-sign text-success"></i>');
            }else{
                $(loading).html('<i class="glyphicon glyphicon-remove-sign text-danger"></i>');
                $(loading).attr('title',callback.message);
            }
        },
        error:function(){
...
function TypeToText(value){
    var text = '';
    $.each(window['window_host_type'], function (k, v) {
        if(v[0] == value){
            text = v[1];
            return
        }
