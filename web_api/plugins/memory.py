...
class Memory(BasePlugins):
    ...
    def process(self, info, host_obj):
        ...
        disk_info = info['data']
        # 获取原数据库中的数据，将槽位也变成集合
        old_db = models.RAM.objects.filter(asset=host_obj)
        # {"0":obj,"1":obj}
        ...
        if create_slot_set:  # 判断create_slot_set有值才是新增
            models.RAM.objects.bulk_create(create_object_list, batch_size=10)
            for item in msg_dict:
                msg = "【新增内存】在%s槽位新增了容量为%s的内存" % (item, msg_dict[item])
                recode_msg_list.append(msg)

        # 删除操作
        if remove_slot_set:
            models.RAM.objects.filter(slot__in=remove_slot_set, asset=host_obj).delete()
            for item in remove_slot_set:
                msg = "【删除内存】在%s槽位删除了容量为%s的内存" % (item, old_db_dict[item].capacity)
                recode_msg_list.append(msg)
